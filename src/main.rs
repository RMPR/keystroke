...
impl Application for TypingTutor {
        ...
        match self.status {
            Status::Ready => {
                if let Message::StartTyping = message {
                    ...
                    // In a real app, you'd focus the text input here using widget IDs and commands
                }
            }
            Status::Typing => {
                if let Message::InputChanged(value) = message {
                    ...
                    if self.current_input.len() >= self.sample_text.len() {
                         ...
                         self.current_input = self.current_input[..self.sample_text.len()].to_string();
             ...
             Status::Finished => {
                 // Allow restarting the test
                  if let Message::StartTyping = message {
                        ...
                        self.results = None;
                        self.start_time = Some(Instant::now());
                        // Again, would ideally refocus the input field
                  }
             }
        }
    ...
    fn view(&self) -> Element<Message> {
        ...
        let mut col = Column::new().spacing(10).padding(20).align_items(iced::Alignment::Center);
        ...
        if let Some(results) = &self.results {
             ...
             if self.status == Status::Finished {
                col = col.push(text("--- Results ---").size(20).style(iced::theme::Text::Default)); // Use theme color
...
fn calculate_results(sample_text: &str, user_input: &str, duration_secs: f64) -> TypingResult {
     ...
     TypingResult { wpm, accuracy, duration: duration_secs }
}
